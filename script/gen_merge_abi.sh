...
# only event abi
jq -s '.[0].abi + .[1].abi + .[2].abi + .[3].abi + .[4].abi + .[5].abi' out/USTB.sol/USTB.json $PROXY_FILE $FOO_FILE $FOO2_FILE $FOO3_FILE $FOO4_FILE > ustbAbi.json
jq -s '.[0].abi + .[1].abi + .[2].abi + .[3].abi + .[4].abi + .[5].abi' out/USCC.sol/USCC.json $PROXY_FILE $FOO_FILE $FOO2_FILE $FOO3_FILE $FOO4_FILE > usccAbi.json
