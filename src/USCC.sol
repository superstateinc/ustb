...
contract USCC is USTB {
     ...
     */
    constructor(address _admin, AllowList _allowList) USTB(_admin, _allowList) {}
    ...
    function transfer(address dst, uint256 amount) public override returns (bool) {
        ...
        if (dst == address(this)) {
            ...
            if (!dstPermissions.state1) revert InsufficientPermissions();
    ...
    function _mintLogic(address dst, uint256 amount) internal override {
    ...
    function _encumber(address owner, address taker, uint256 amount) internal override {
        ...
        if (!permissions.state1) revert InsufficientPermissions();
